...
from core.langchain_translator import LangChainTranslator, Provider, TranslationError
...
def get_translator(
    ...
    return LangChainTranslator(provider=provider, model=model, timeout=timeout)
...
def main() -> None:
    ...
    request, provider, model = render_sidebar(settings.default_provider, settings.model)
    if not settings.has_api_key(provider):
